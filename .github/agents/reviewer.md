...
3. **Judge adversarially:** Does the diff satisfy the issue's **Scope + Acceptance** (nothing missing/extra)? Is it the **smallest correct** change? Regressions, anti-patterns, or violations of the PRD/ADR decisions? Does `xcodebuild`/CI pass on the PR?
