...
- **G7 — Auth.** Bearer-token via a **system-browser handoff** (ASWebAuthenticationSession → connect endpoint → `superset://auth/callback?token=` → Keychain → `Authorization: Bearer`); mint a short-lived RS256 JWT from `/api/auth/token` for relay/host calls (ADR-0005).
- **G8 — Register as device type `visionos`** in **`deviceTypeValues`** (the load-bearing enum: gates `device.registerDevice` → `device_presence`, the actual targeting path). `v2ClientTypeValues`/`ensureClient` is inert today; add `'visionos'` there too, defensively. Append the value **last** to avoid a destructive enum recreate (`packages/db/src/schema/enums.ts`).
...
### Auth (ADR-0005)
- **System-browser handoff:** `ASWebAuthenticationSession` (real Safari → Google/GitHub OAuth allowed) → a connect endpoint analogous to the desktop's `/api/auth/desktop/connect` → `superset://auth/callback?token=…&expiresAt=…&state=…` → store the **session token** in **Keychain** → `Authorization: Bearer <token>` on every API/tRPC call (`apps/api/src/trpc/context.ts` `getSession` and chat `requireAuth` honor it; mirrors `@better-auth/expo`).
...
- **Schema:** add `'visionos'` to **`deviceTypeValues`** (the load-bearing enum — gates `device.registerDevice` → `device_presence`, the real targeting path; `agent_commands` deliver to `deviceId`). Add to `v2ClientTypeValues` too, defensively (it's inert — `ensureClient` has zero callers). **Append last** (Drizzle else emits a destructive enum recreate). Additive migration via `drizzle-kit generate` (maintainer-run; never hand-edit `packages/db/drizzle/`).
