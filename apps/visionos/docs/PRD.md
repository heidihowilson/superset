...
- **G7 — Auth.** System-browser handoff (ASWebAuthenticationSession, real Safari) → store the better-auth **session credential** in Keychain → replay it (`Cookie` header per `@better-auth/expo`, or `Authorization: Bearer` via the enabled `bearer()` plugin); mint a short-lived RS256 JWT from `/api/auth/token` for relay/host calls. Mirror the desktop `/api/auth/desktop/connect` handoff (ADR-0005).
...
### Auth (ADR-0005)
- **System-browser handoff:** `ASWebAuthenticationSession` (real Safari → Google/GitHub OAuth allowed) → the desktop's `/api/auth/desktop/connect` flow (exists; redirects to `/auth/desktop/success` — **confirm its exact token-delivery hop and mirror it**) → store the better-auth **session credential** in **Keychain** → replay it on every API/tRPC call as a **`Cookie` header** (proven by `apps/mobile` `@better-auth/expo` `getCookie()`) or **`Authorization: Bearer`** (the `bearer()` plugin is enabled, `packages/auth/src/server.ts:778`); `apps/api/src/trpc/context.ts` `getSession` and chat `requireAuth` honor both.
